...
impl std::str::FromStr for IdeType {
    ...
    fn from_str(s: &str) -> Result<Self> {
        match s.to_lowercase().as_str() {
            "zed" => Ok(IdeType::Zed),
            _ => anyhow::bail!("Unsupported IDE: {}. Supported: zed", s),
        }
    }
}
...
pub fn generate(ide: IdeType, output: Option<String>) -> Result<()> {
    ...
    if !manager.exists() {
        anyhow::bail!(
            "No .mcp-connect.json found. Run 'mcp-connect init' first."
        );
    }
...
fn generate_zed_config(
    _config: &mcp_types::ConnectConfig,
    output: Option<&str>,
) -> Result<()> {
    ...
    let (existing_config, is_new) = if output_path.exists() {
        let content = fs::read_to_string(&output_path)
            .context("Failed to read existing Zed config")?;
    ...
    println!("✓ Generated Zed configuration at: {}", output_path.display());
    println!("\n📝 Configuration added:");
